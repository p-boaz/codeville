...
The renderer never receives prompts, source code, diffs, command output, or raw final messages through the village channel. What it does receive is specific: command lines (truncated, never their output), the names of files read and edited, search queries, diff stats, and validated `CompletionDebrief` objects — these feed the desk's **Village feed**, one stream across all builders, each row named with its repository and task. Wall mode never renders the desk, so the projectable wall surface still shows only phases and counts. Debrief strings are length-bounded and reject URLs, emails, shell/code syntax, and common secret shapes — file paths are allowed, so builders can say exactly what they changed. Working directories appear only in the local approval dialog when a user decision is required.
