...
const bridge: CodevilleBridge = {
  ...
  prepareDemoProject: () => ipcRenderer.invoke('project:demo'),
  startSession: (input) => ipcRenderer.invoke('session:start', input),
  interruptSession: () => ipcRenderer.invoke('session:interrupt'),
  ...
  onVillageEvent: (listener) => {
    const wrapped = (_event: Electron.IpcRendererEvent, villageEvent: VillageEvent) => listener(villageEvent);
