...
5. Click **Start all builders**, review the exact five-repository/task preflight, then click **Confirm and start builders**. Five real Codex threads repair five isolated repositories simultaneously. A typical verified run took 44–46 seconds; model load can vary.
...
10. A missing result marker displays **Result needs review** and leaves the completed-session count unchanged.
7. Quit and reopen to verify assignments, UUID identity, progression, and safe debriefs restore. Task drafts intentionally do not restore.
