...
11b. To run **two builders on one repository**, choose the same repository for a second lot and pick **Add a second workshop** in the dialog. Both builders work concurrently in separate scaffolds and appear together on one shared parcel — two houses, one nameplate; each improvement is inspected and landed independently.
